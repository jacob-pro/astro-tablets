...
from astro_tablets.generate.eclipse import EclipseTool, TimeUnit, lunar_eclipse_on_date
from astro_tablets.util import TimeValue


def plot_eclipse(data: AstroData, t0: Time, dest: str):
    times, types, details = eclipselib.lunar_eclipses(
        data.timescale.tt_jd(t0.tt - 1), data.timescale.tt_jd(t0.tt + 1), data.ephemeris
    )
    ...
    ax1.set_title(
        "{} (UTC+3) - {}".format(
            TimeValue(times[0].tt).string(data.timescale),
    ...
    ax1.plot(*zip(*points))
    ax1.axhline(
        y=details["penumbra_radius_radians"][0] + moon_radius,
        color="g",
        label="Penumbral",
    )
    ax1.axhline(
        y=details["umbra_radius_radians"][0] + moon_radius, color="y", label="Partial"
    )
    ax1.axhline(
        y=details["umbra_radius_radians"][0] - moon_radius, color="r", label="Total"
    )
    ax1.legend()
    ...
    if again.type == "Total":
        t = "1:2 = {:.2f} UŠ   2:3 = {:.2f} UŠ   3:4 = {:.2f} UŠ   (1:4 = {:.2f} UŠ)".format(
            phases.onset, phases.maximal, phases.clearing, phases.sum
        )
        ax2.text(0, 0, t, fontsize=11)
    elif again.type == "Partial":
        t = "1:M = {:.2f} UŠ   M:4 = {:.2f} UŠ   (1:4 = {:.2f} UŠ)".format(
            phases.onset, phases.clearing, phases.sum
        )
        ax2.text(0, 0, t, fontsize=11)
