...
class BM32234(AbstractTablet):
    def year_14_month_6(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        return [
            LunarEclipseQuery(
                self.db,
                FirstContactTime(0, FirstContactRelative.AFTER_SUNRISE),
    ...
    def year_14(self, nisan_1: float) -> List[MonthResult]:
        ...
        month_12 = self.repeat_month_with_alternate_starts(
            nisan_1, 12, self.year_14_month_12
        )
    ...
    def year_31_month_unknown(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        eclipse_pos = EclipsePosition(
            LIBRA.central_star, 0, LIBRA.radius, EclipticPosition.AHEAD
        )
        eclipse_time = FirstContactTime(t, FirstContactRelative.AFTER_SUNSET)
        eclipse = LunarEclipseQuery(
            self.db,
            eclipse_time,
    ...
    def year_32_month_12(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        return [
            LunarEclipseQuery(
                self.db, None, ExpectedEclipseType.UNKNOWN, None, None, range
            )
        ]

    def year_32(self, nisan_1: float) -> List[MonthResult]:
        month_6 = self.repeat_month_with_alternate_starts(
            nisan_1, 6, self.year_32_month_6
        )
        month_12 = self.repeat_month_with_alternate_starts(
            nisan_1, 12, self.year_32_month_12
        )
        return [month_6, month_12]
    ...
    def year_50_month_7(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        location = EclipsePosition(
            ARIES.central_star, 0, ARIES.radius, EclipticPosition.BEHIND
        )
        res1 = LunarEclipseQuery(
            self.db,
            eclipse_time,
    ...
    def year_50(self, nisan_1: float) -> List[MonthResult]:
        month_7 = self.repeat_month_with_alternate_starts(
            nisan_1, 7, self.year_50_month_7
        )
        month_13 = self.repeat_month_with_alternate_starts(
            nisan_1, 13, self.year_50_month_13, name="Addaru II"
        )
    ...
    def year_68_month_7(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        res1 = LunarEclipseQuery(
            self.db, eclipse_time, ExpectedEclipseType.UNKNOWN, None, location, range
        )
        return [res1]

    def year_68(self, nisan_1: float) -> List[MonthResult]:
        month_7 = self.repeat_month_with_alternate_starts(
            nisan_1, 7, self.year_68_month_7
        )
        return [month_7]
