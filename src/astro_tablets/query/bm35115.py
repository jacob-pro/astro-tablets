...
class BM35115(AbstractTablet):
    ...
    def year_0(self, nisan_1: float) -> List[MonthResult]:
        month_2 = self.repeat_month_with_alternate_starts(
            nisan_1, 2, self.year_0_month_2
        )
        month_8 = self.repeat_month_with_alternate_starts(
            nisan_1, 8, self.year_0_month_8
        )
    ...
    def year_2(self, nisan_1: float) -> List[MonthResult]:
        month_1 = self.repeat_month_with_alternate_starts(
            nisan_1, 1, self.year_2_month_1
        )
        return [month_1]
    ...
    def year_18_month_2(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        return [
            LunarEclipseQuery(
                self.db,
                FirstContactTime(t, FirstContactRelative.BEFORE_SUNSET),
                ExpectedEclipseType.UNKNOWN,
                None,
                None,
                SearchRange.any_day(month),
    ...
    def year_18(self, nisan_1: float) -> List[MonthResult]:
        month_2 = self.repeat_month_with_alternate_starts(
            nisan_1, 2, self.year_18_month_2
        )
        month_8 = self.repeat_month_with_alternate_starts(
            nisan_1, 8, self.year_18_month_8
        )
        return [month_2, month_8]
    ...
    def year_36_month_3(self, month: List[BabylonianDay]) -> List[AbstractQuery]:
        ...
        res1 = LunarEclipseQuery(
            ...
            CompositePhaseTiming(20),
            location,
            day15,
        )
        return [res1]

    def year_36(self, nisan_1: float) -> List[MonthResult]:
        month_3 = self.repeat_month_with_alternate_starts(
            nisan_1, 3, self.year_36_month_3
        )
        return [month_3]
