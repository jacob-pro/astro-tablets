...
def plot_eclipse_time_of_day_score(dest: str):
    ...
    ys = list(map(lambda x: LunarEclipseQuery.eclipse_time_of_day_score(eclipse, FirstContactTime(x,
                  ...
                  xs))
    ax1.plot(xs, ys, label="Regular (λ = {})".format(LunarEclipseQuery.REGULAR_TIME_TOLERANCE), color='g')
    ...
    ys = list(map(lambda x: LunarEclipseQuery.eclipse_time_of_day_score(eclipse, FirstContactTime(x,
                  ...
                  xs))
    ax1.plot(xs, ys, label="High (λ = {})".format(LunarEclipseQuery.HIGH_TIME_TOLERANCE), color='b')
